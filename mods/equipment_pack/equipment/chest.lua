...
function equipment.chest.get_chest_formspec(pos)
...
function equipment.chest.chest_lid_obstructed(pos)
...
function equipment.chest.chest_lid_close(pn)
	local chest_open_info = equipment.chest.open_chests[pn]
	local pos = chest_open_info.pos
	local sound = chest_open_info.sound
	local swap = chest_open_info.swap

	equipment.chest.open_chests[pn] = nil
	for k, v in pairs(equipment.chest.open_chests) do
...
equipment.chest.open_chests = {}

minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "equipment:chest" then
		return
	end
	...
	if not equipment.chest.open_chests[pn] then
		return
	end

	equipment.chest.chest_lid_close(pn)
	return true
end)

minetest.register_on_leaveplayer(function(player)
	local pn = player:get_player_name()
	if equipment.chest.open_chests[pn] then
		equipment.chest.chest_lid_close(pn)
	end
end)

function equipment.chest.register_chest(prefixed_name, d)
	...
	if def.protected then
		...
		def.on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
			...
			if not equipment.chest.chest_lid_obstructed(pos) then
			...
			minetest.after(0.2, minetest.show_formspec,
					...
					"equipment:chest", equipment.chest.get_chest_formspec(pos))
			equipment.chest.open_chests[clicker:get_player_name()] = { pos = pos,
		...
		def.on_key_use = function(pos, player)
			...
			minetest.show_formspec(
				...
				equipment.chest.get_chest_formspec(pos)
			)
		end
...
equipment.chest.register_chest("equipment:chest", {
	description = S("Chest"),
	tiles = {
		"equipment_chest_top.png",
		"equipment_chest_top.png",
		"equipment_chest_side.png",
		"equipment_chest_side.png",
		"equipment_chest_front.png",
		"equipment_chest_inside.png"
	},
...
equipment.chest.register_chest("equipment:chest_locked", {
...
minetest.register_craft( {
	type = "shapeless",
	output = "equipment:chest_locked",
	recipe = {"equipment:chest", "mineral:steel_ingot"},
})
