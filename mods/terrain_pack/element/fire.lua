...
local fire_node = {
	drawtype = "firelike",
	tiles = {{
		name = "element_basic_flame_animated.png",
		animation = {
	...
	inventory_image = "element_basic_flame.png",
	paramtype = "light",
	light_source = 13,
	walkable = false,
	buildable_to = true,
...
minetest.register_node("element:basic_flame", flame_fire_node)

-- Permanent flame node
local permanent_fire_node = table.copy(fire_node)
permanent_fire_node.description = S("Permanent Fire")

minetest.register_node("element:permanent_flame", permanent_fire_node)
...
if flame_sound then
	...
	function element.update_player_sound(player)
		...
		local fpos, num = minetest.find_nodes_in_area(
			areamin,
			areamax,
			{"element:basic_flame", "element:permanent_flame"}
		)
		-- Total number of flames in radius
		local flames = (num["element:basic_flame"] or 0) +
			(num["element:permanent_flame"] or 0)
		-- Stop previous sound
	...
	minetest.register_globalstep(function(dtime)
		...
		for n = 1, #players do
			element.update_player_sound(players[n])
		end
	end)
...
function element.update_sounds_around() end
...
if fire_enabled then
	...
	minetest.register_abm({
		...
		action = function(pos)
			...
			if p then
				minetest.set_node(p, {name = "element:basic_flame"})
			end
		end
	})
	...
	minetest.register_abm({
		label = S("Remove flammable nodes"),
		nodenames = {"element:basic_flame"},
